...
// --- Types based on Data Dictionary [cite: 2, 3, 5, 6] ---

interface PatientProfile {
    // Fields from Patient_tbl [cite: 3]
    patient_id: number;
    f_name: string;
    l_name: string;
    m_name: string;
    ...
    gender: 'Male' | 'Female' | 'LGBTQIA+' | 'Prefer Not to Say'; // Enum [cite: 3]
    email: string;
    address: string;
    blood_type: 'A+' | 'A-' | 'AB+' | 'AB-' | 'B+' | 'B-' | 'O+' | 'O-' | 'Unspecified'; // Enum [cite: 3]
    pri_contact_no: string; // Varchar(15)
    sec_contact_no: string; // Varchar(15)
    account_status: 'Pending' | 'Active' | 'Inactive' | 'Suspended'; // Enum [cite: 3]
    created_at: string; // Timestamp
    image_url: string;
}

interface EmergencyContact {
    // Fields from Emergency_contact_tbl [cite: 6]
    ec_f_name: string;
    ec_l_name: string;
    ec_contact_no: string;
    ec_relationship: 'Parent' | 'Child' | 'Relative' | 'Spouse' | 'Friend' | 'Sibling' | 'Guardian' | 'Others' | 'Unspecified'; // Enum [cite: 6]
    ec_email: string;
}
...
const MOCK_PATIENT: PatientProfile = {
    ...
    blood_type: "O+",
    pri_contact_no: "09171234567",
    sec_contact_no: "09187654321",
...
const MOCK_EMERGENCY: EmergencyContact = {
    ec_f_name: "Jane",
    ec_l_name: "Doe",
    ec_contact_no: "09201112222",
    ec_relationship: "Spouse",
    ec_email: "janedoe@example.com"
};

export default function PatientProfilePage() {
    const [isEditing, setIsEditing] = useState(false);
    const [profile, setProfile] = useState<PatientProfile>(MOCK_PATIENT);
    const [ec, setEc] = useState<EmergencyContact>(MOCK_EMERGENCY);
    ...
    // Handle Save
    const handleSave = () => {
        // In a real app, this would be an API PUT request to update Patient_tbl and Emergency_contact_tbl
        console.log("Saving Profile:", profile);
        console.log("Saving EC:", ec);
        setIsEditing(false);
    };

    // Helper for Enum Colors
                            ...
                            <div className="grid grid-cols-2 gap-4 text-left">
                                ...
                                <div>
                                    ...
                                    <div className="flex items-center gap-1 text-sm font-medium">
                                        <Droplet className="w-3 h-3 text-red-500 fill-red-500" /> {profile.blood_type}
                                    </div>
                                </div>
                            </div>
                        </CardContent>
                        ...
                        <TabsContent value="personal">
                            <Card>
                                <CardHeader>
                                    ...
                                    <CardDescription>
                                        Official identification details stored in Patient Records[cite: 3].
                                    </CardDescription>
                                </CardHeader>
                                <CardContent className="space-y-4">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            {isEditing ? (
                                                <Select value={profile.gender} onValueChange={(val: any) => setProfile({...profile, gender: val})}>
                                                    ...
                                                    <SelectContent>
                                                        {/* Enum Values from Data Dictionary [cite: 3] */}
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            {isEditing ? (
                                                <Select value={profile.blood_type} onValueChange={(val: any) => setProfile({...profile, blood_type: val})}>
                                                    <SelectTrigger><SelectValue /></SelectTrigger>
                                                    <SelectContent>
                                                        {/* Enum Values from Data Dictionary [cite: 3] */}
                                                ...
                                                <Input disabled value={profile.blood_type} />
                                            )}
                                        </div>
                                    </div>
                                </CardContent>
                        ...
                        <TabsContent value="contact">
                            <Card>
                                <CardHeader>
                                    ...
                                    <CardDescription>
                                        How we can reach you for appointments and notifications[cite: 3].
                                    </CardDescription>
                                </CardHeader>
                                <CardContent className="space-y-4">
                                    <div className="space-y-2">
                        ...
                        <TabsContent value="emergency">
                            <Card>
                                <CardHeader>
                                    ...
                                    <CardDescription>
                                        Primary contact person in case of emergency[cite: 6].
                                    </CardDescription>
                                </CardHeader>
                                <CardContent className="space-y-4">
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            {isEditing ? (
                                                <Select value={ec.ec_relationship} onValueChange={(val: any) => setEc({...ec, ec_relationship: val})}>
                                                    <SelectTrigger><SelectValue /></SelectTrigger>
                                                    <SelectContent>
                                                        {/* Enum Values from Data Dictionary [cite: 6] */}
