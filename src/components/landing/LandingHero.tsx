            ...
            <div className="relative grid items-center gap-10 lg:grid-cols-[minmax(0,1fr)_minmax(0,520px)]">
                ...
                <div className="relative hidden lg:block">
                    <div className="mx-auto aspect-4/5 w-full max-w-md overflow-hidden  ">
                        <img src={imgDentist} alt="Dentist with patient" className="h-full w-full object-contain" />
                    </div>
                    ...
                    <div className="absolute inset-x-0 top-8 flex justify-between">
                        {[imgBubble1, imgBubble2].map((img, index) => (
                            <div key={img} className={cn('size-32 rounded-full border-4 border-card shadow-lg animate-float', index === 0 ? 'translate-x-6' : '-translate-x-6')}>
                    ...
                    <div className="absolute inset-x-0 bottom-0 flex justify-between">
                        {[imgBubble3, imgBubble4].map((img, index) => (
                            <div key={img} className={cn('size-28 rounded-full border-4 border-card shadow-lg animate-float', index === 0 ? 'translate-y-6' : '-translate-y-4')}>
